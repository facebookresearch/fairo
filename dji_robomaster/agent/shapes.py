...
def hollow_triangle(
    ...
    """Construct an empty isosceles triangle with a given half base length
    (b/2).

    The construction is restricted to having stepped blocks for the
    sides other than base. Hence the height is (total base length)//2 +
    1.
    """
    assert size > 0
...
def arch(size=3, distance=11, bid=DEFAULT_IDM, orient="xy", labelme=False, **kwargs):
    """Arch is a combination of 2 parallel columns, where the columns are
    connected by a stepped roof.
...
def get_bounds(S):
    """S should be a list of tuples, where each tuple is a pair of (x,y,z) and
    ids."""
...
def arrange(arrangement, schematic=None, shapeparams={}):
    """This function arranges an Optional schematic in a given arrangement and
    returns the offsets."""
