...
var Builder = function () {
  ...
  _createClass(Builder, [{
    ...
    value: function checkAnd(path, value, otherKeys) {
      if ("production" !== process.env.NODE_ENV) {
        var keys = [this.key(path)].concat(_toConsumableArray(otherKeys));
    ...
    value: function checkType(path, value) {
      if ("production" !== process.env.NODE_ENV) {
        switch (this.type) {
          ...
          default:
            this.invariant(_typeof(value) === this.type, "Must be a ".concat(this.type, "."), path);
            break;
        }
      }
    }
    ...
    value: function only() {
      if ("production" !== process.env.NODE_ENV) {
        this.invariant(_typeof(this.defaultValue) === this.type, "Only requires a default ".concat(this.type, " value."));
      }
    ...
    value: function runChecks(path, initialValue, struct) {
      ...
      if ("production" !== process.env.NODE_ENV) {
        this.checks.forEach(function (checker) {
          ...
          (_checker$callback = checker.callback).call.apply(_checker$callback, [_this, path, value].concat(_toConsumableArray(checker.args)));
        });
      }
...
export function custom(callback) {
