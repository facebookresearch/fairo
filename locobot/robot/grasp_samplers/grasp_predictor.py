...
class Predictors:
    """This class contains functionality to sample grasp patches from the scene
    image."""
    ...
    def graspNet_grasp(self, patch_size=300, num_samples=128):
        ...
        patch_Is_resized = np.zeros(
            (num_samples, self.grasp_obj.image_size, self.grasp_obj.image_size, self.img_c)
        )
        for looper in xrange(num_samples):
            ...
            while isWhiteFlag == 1:
                ...
                patch_Is[looper] = self.img[h_b : h_b + patch_size, w_b : w_b + patch_size]
