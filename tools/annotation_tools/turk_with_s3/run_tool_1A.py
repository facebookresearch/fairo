...
1. Create HITs for each input command using tool 1A template.
2. Continuously check for completed assignments, fetching results in batches.
3. Collate turk output data with the input job specs.
4. Postprocess datasets to obtain well formed action dictionaries.
"""

# Load input commands and create a separate HIT for each row
rc = subprocess.call(["python create_jobs.py"], shell=True)
if rc != 0:
