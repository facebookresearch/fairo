...
def process_repeat_dict(d):
    ...
    if d["loop"] == "repeat_all":
        ...
        return repeat_dict
    if d["loop"] == "forever":
        return {"terminate_condition": {"condition_type": "NEVER"}}
    if d["loop"] == "repeat_until":
        ...
        if "adjacent_to_block_type" in processed_d:
            ...
            return {"terminate_condition": {"condition_span": processed_d["condition_span"]}}
    ...
    for d, val in dicts.items():
        ...
        for k, v in d.items():
            if type(v[1]) == list:
                ...
                if "terminate_condition" in v[1]:
                    new_v = {}
                    new_v["terminate_condition"] = {}
                    x = {}

                    if "condition_type" in v[1]["terminate_condition"]:
                        x["condition_type"] = v[1]["terminate_condition"]["condition_type"]
                    new_vals = []
                    if "block_type" in v[1]["terminate_condition"]:
                        for item in v[1]["terminate_condition"]["block_type"]:
                            new_vals.append(words[item[0]])
                        x["block_type"] = new_vals
                    elif "condition_span" in v[1]["terminate_condition"]:
                        for item in v[1]["terminate_conditions"]["condition_span"]:
                            new_vals.append(words[item[0]])
                        x["condition_span"] = new_vals
                    new_v["terminate_condition"] = x
