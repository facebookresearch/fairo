...
def is_annot_validfn(annot):
    def load_semantic_json(scene):
        habitat_semantic_json = os.path.join('/checkpoint/apratik/replica', scene, 'habitat', 'info_semantic.json')
...
def log_job_start(args, jobs):
    with open(f"/checkpoint/{os.environ.get('USER')}/jobs/active_vision/pipeline/dec_class_slurm_launch_start.txt", 'a') as f:
