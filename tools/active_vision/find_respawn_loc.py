...
from common_utils import is_annot_validfn_class, is_annot_validfn_inst, log_time
...
def get_target(traj_path, img_indx, is_annot_validfn, base_pos):
    ...
    c, _ = get_center_of_larger_mask(src_label, is_annot_validfn)
    if c is None:
        return None
        ...
        for traj_path in get_valid_trajectories(baseline_root)[:num_traj]:
            if traj_path.split('/')[-1].isdigit():
                ...
                for setting in ['class', 'instance']:
                    ...
                    def job_unit(traj_path, out_dir, traj_id, annot_fn, setting):
                    ...
                    job = executor.submit(job_unit, traj_path, out_dir, traj_id, annot_fn, setting)
                    jobs.append(job)
