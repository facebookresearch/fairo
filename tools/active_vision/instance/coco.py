...
class CocoCreator:
    ...
    def __init__(self, root_data_dir, semantic_json_root, instance_ids, out_dir):
    ...
    def create_coco(self, scene, coco_file_name, pct, src_img_ids=[], prop_length=0):
        hsd = self.load_semantic_json(scene)
        self.create_metadata(hsd)
        self.create_annos(hsd, scene, pct, src_img_ids, prop_length)
    ...
    def save_visual_dataset(self, coco_file_name, scene):
        ...
        save_dir = os.path.join(self.out_dir, f"coco_visuals_{coco_file_name.split('/')[-1].split('.')[0]}")
...
def run_coco(root_data_dir, instance_ids, src_img_ids, p, out_json, out_dir):
    cbase = CocoCreator(root_data_dir, semantic_json_root, instance_ids, out_dir)
    cbase.create_coco(
        scene='apartment_0', 
        coco_file_name=os.path.join(out_dir, out_json),
        pct=1,
        src_img_ids=src_img_ids,
        prop_length=p
    )