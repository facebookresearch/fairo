...
from agents.swarm_config import get_swarm_interpreter, get_memory_handlers_dict
from droidlet.dialog.swarm_dialogue_manager import SwarmDialogueManager
from agents.swarm_worker_process import SwarmWorkerWrapper
...
class SwarmMasterWrapper():
    ...
    def init_workers(self, worker_agents, opts):
        ...
        self.swarm_workers = [
            SwarmWorkerWrapper(opts, task_map=task_map, disable_perception_modules=disable_perception_modules,
                               idx=i + 1) for i in range(self.num_workers)]
