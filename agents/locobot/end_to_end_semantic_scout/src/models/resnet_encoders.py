    ...
    def __init__(
        ...
        if not self.is_blind:
            ...
            final_spatial = np.array(
                [math.ceil(d * self.backbone.final_spatial_compress) for d in spatial_size]
            )
...
class TorchVisionResNet50(nn.Module):
    ...
    def __init__(self, observation_space, output_size, device, spatial_output: bool = False):
        super().__init__()
        self.device = device
