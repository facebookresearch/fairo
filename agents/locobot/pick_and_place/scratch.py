...
# rospy.init_node('debug')
# dpt_cam = RosCamera('/camera/aligned_depth_to_color', buffer_size=1)
# pcd3 = np.rot90(dpt_cam.depth_to_xyz(np.rot90(depth, k=1, axes=(1, 0))), k=1, axes=(0, 1))
# show_point_cloud(pcd3, rgb, orig=np.zeros(3))

rospy.init_node('debug')
dpt_cam = RosCamera('/camera/aligned_depth_to_color', buffer_size=1)
pcd4 = np.rot90(dpt_cam.depth_to_xyz(np.rot90(depth, k=1, axes=(0, 1))), k=3, axes=(1, 0))
show_point_cloud(pcd4, rgb, orig=np.zeros(3))
