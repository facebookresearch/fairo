...
class SpideyGripperServer(polymetis_pb2_grpc.GripperServerServicer):
    """gRPC server that exposes a custom SpideyHand gripper controls to the client
    Communicates with the gripper through serial
    """
...
class GripperServerLauncher:
    def __init__(self, ip="localhost", port="50052", comport="/dev/ttyACM2"):
        ...
        polymetis_pb2_grpc.add_GripperServerServicer_to_server(
            SpideyGripperServer(comport), self.server
        )
        self.server.add_insecure_port(self.address)

    def run(self):
        self.server.start()
        print(f"SpideyHand gripper server running at {self.address}.")
