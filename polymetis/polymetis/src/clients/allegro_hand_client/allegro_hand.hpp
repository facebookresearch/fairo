...
// Joint and Torque state vectors are indexed as [finger_idx * kNJoint + joint_idx]
// Fingers are indexed in order [index, middle, ring, thumb] (there is no 5th finger)
class AllegroHand {
 public:
    // n.b. The firmware appears to only support periodic broadcast of joint position
    ...
    // Returns true if all the fingers have received state updates since last reset
...
class AllegroHandImpl : public AllegroHand {
 ...
 private:
    bool send(unsigned char msg_id, char data_len = 0, const void* data = NULL, bool expect_return = false);
...
class MockAllegroHand : public AllegroHand {
 public:
    ...
    bool allStatesUpdated() const override {
        return all_updated_;
    }

    void resetStateUpdateTracker() override {
        all_updated_ = false;
    }
