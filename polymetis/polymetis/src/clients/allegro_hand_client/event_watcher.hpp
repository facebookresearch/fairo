...
class EventDataT {
 public:
    ...
    friend std::ostream& operator<<(std::ostream& os, const EventDataT<Clock>& data) {
    ...
    inline double secondsSinceCheckpoint() const {
        return std::chrono::duration_cast<std::chrono::duration<double>>
                (Clock::now() - last_checkpoint_time_).count();
    }
...
class EventWatcher {
 public:
    ...
    void checkpoint();
    friend std::ostream& operator<<(std::ostream& os, const EventWatcher& watcher);
 private:
