...
def get_task_data(agent):
    ...
    if str(reexplore_id) not in reex.keys():
        return None
    
    task_data = {
        'spawn_pos': reex[str(reexplore_id)]['spawn_pos'],
        'base_pos': reex[str(reexplore_id)]['base_pos'],
        'target': {'xyz': reex[str(reexplore_id)]['target'], 'label': 'object'},
        'data_path': f'{agent.opts.data_store_path}/{reexplore_id}',
        'root_data_path': f'{agent.opts.data_store_path}/{reexplore_id}',
    }
