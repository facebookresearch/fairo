...
def main(batch_id: int):
    ...
    not_done = [x for x in partially_anno_scenes 
                if not x["inst_seg_tags"] 
                or x["inst_seg_tags"][0]["tags"][0] in SHAPE_NAMES]
    clean_anno_scenes = [x for x in partially_anno_scenes 
                        if x["inst_seg_tags"] 
                        and x["inst_seg_tags"][0]["tags"][0] not in SHAPE_NAMES]
    ...
    print(f"{unannotated_filename} can be combined with other labeled scene lists and used to launch a new annotation job")
