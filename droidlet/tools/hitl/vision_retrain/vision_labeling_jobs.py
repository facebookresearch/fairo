...
VIS_LABELING_LISTENER_POLL_TIME = 30
...
class VisionLabelingJob(DataGenerator):
    ...
    def __init__ (self, SL: int, H: int, GROUND_DEPTH: int, MAX_NUM_SHAPES: int, NUM_SCENES: int, MAX_HOLES: int, timeout: float = -1) -> None:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            scene_gen_cmd = "python3 " + \
                ...
                " --SL=" + str(self._SL) + \
                " --H=" + str(self._H) + \
                " --GROUND_DEPTH=" + str(self._GROUND_DEPTH) + \
                " --MAX_NUM_SHAPES=" + str(self._MAX_NUM_SHAPES) + \
                " --NUM_SCENES=" + str(self._NUM_SCENES) + \
                " --MAX_NUM_GROUND_HOLES=" + str(self._MAX_HOLES) + \
            ...
            while not (datetime.now().timestamp() - now > SCENE_GEN_TIMEOUT) and scene_gen.poll() is None:
                logging.info(f"Scene generation script still running...Remaining time: {int(SCENE_GEN_TIMEOUT - (datetime.now().timestamp() - now))} seconds")
            ...
            upload_key = self._batch_id + "/vision_labeling_results/original_scene_list.json"
            ...
            with open("conf/labeling.yaml", "r") as stream:
                ...
                task_name = "ca-vis-label" + str(self._batch_id)
            ...
            while not self.check_is_timeout() and job_launch.poll() is None:
                logging.info(f"Vision Labeling Job [{self._batch_id}] still running...Remaining time: {self.get_remaining_time()}")
            ...
            results_csv = self._batch_id + ".csv"
            ...
            upload_key = self._batch_id + "/vision_labeling_results/" + results_csv
            ...
            upload_key = self._batch_id + "/vision_labeling_results/scene_list.json"
            logging.info(f"Uploading job results to S3: {upload_key}")
            response = s3.upload_file(json.dumps(scene_list), S3_BUCKET_NAME, upload_key)
            if response: logging.info("S3 response: " + response)
        ...
        while not self.check_is_finished():
            try:
                with open('scene_list.json', 'wb') as js:
                    s3.download_fileobj(f"{S3_BUCKET_NAME}", f"{batch_id}/vision_labeling_results/scene_list.json", js)
            except:
                logging.info(
                ...
                #TODO TORCH THIS
                logging.info("SCENE LIST FOUND, EXITING FOR DEBUG")
                self.set_finished()
                return

                logging.info(f"Vision labeling data found, downloading new labeled data")
...
if __name__ == "__main__":
    runner = TaskRunner()
    lj = VisionLabelingJob(SL=18, H=13, GROUND_DEPTH=4, MAX_NUM_SHAPES=4, NUM_SCENES=1, MAX_HOLES=3, timeout=30)
    batch_id = lj.get_batch_id()
    listener = VisionLabelingListener(batch_id, 30)
