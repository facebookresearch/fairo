...
from droidlet.tools.artifact_scripts.fetch_artifacts_from_aws import (
    fetch_models_from_aws,
    fetch_datasets_from_aws,
    fetch_test_assets_from_aws,
)
...
def compare_checksum_try_download(agent=None, local_checksum_file=None, artifact_name=None):
    ...
    latest_checksum_file = os.path.join(
        ROOTDIR, "droidlet/tools/artifact_scripts/tracked_checksums/" + artifact_name_file + ".txt"
    )
...
def try_download(agent=None, artifact_name=None, latest_checksum=None):
    ...
    print(
        "Local %r directory is out of sync. Downloading latest. Use --dev to disable downloads."
        % artifact_name
    )
    print(
        "*********************************************************************************************"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_mode", help="Is this is given, download robot test assets", action="store_true"
    )
