...
from droidlet.tools.active_vision.common_utils import EarlyStoppingException
...
class LossEvalHook(HookBase):
    ...
    def update_val_loss_window(self, loss: float) -> None:
        """
    ...
    def after_step(self):
        ...
        if is_final or (self._period > 0 and next_iter % self._period == 0):
            ...
            print(f"Iter {self.trainer.iter} validation losses.mean {l}")
            with open(os.path.join(self.cfg.OUTPUT_DIR, "validation_results.txt"), "a") as f:
                f.write(
                    f"lr {self.cfg.SOLVER.BASE_LR} warmup {self.cfg.SOLVER.WARMUP_ITERS} iter {self.trainer.iter} validation loss {l}\n"
                )
        ...
        hooks.insert(
            ...
            LossEvalHook(
                ...
                build_detection_test_loader(
                    self.cfg, self.cfg.DATASETS.TEST[0], DatasetMapper(self.cfg, is_train=True)
                ),
            ),
        )
        print(f"hooks {len(hooks), hooks}")
...
class COCOTrain:
    ...
    def reset(self):
        ...
        self.register_json(
            self.train_data, self.data["train"]["json"], self.data["train"]["img_dir"]
        )
        if "val" in self.data.keys():
            self.val_data = self.data["dataset_name"] + "_val"
            self.register_json(
                self.val_data, self.data["val"]["json"], self.data["val"]["img_dir"]
            )
        ...
        if "test" in self.data.keys():
            ...
            self.register_json(
                self.test_data, self.data["test"]["json"], self.data["test"]["img_dir"]
            )
    ...
    def vis(self):
        ...
        for d in random.sample(dataset_dicts, 2):
            ...
            visualizer = Visualizer(
                img[:, :, ::-1], metadata=MetadataCatalog.get(self.train_data), scale=0.5
            )
    ...
    def train(self):
        ...
        cfg.SOLVER.STEPS = tuple(
            [100 * (i + 1) for i in range(100) if 100 * (i + 1) < cfg.SOLVER.MAX_ITER]
        )

        cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE = 128
        print(f"classes {MetadataCatalog.get(self.train_data)}")
        cfg.MODEL.ROI_HEADS.NUM_CLASSES = len(
            MetadataCatalog.get(self.train_data).get("thing_classes")
        )
        cfg.OUTPUT_DIR = os.path.join(
            self.data["out_dir"],
            "training",
            str(cfg.SOLVER.MAX_ITER),
            str(cfg.SOLVER.BASE_LR),
            str(cfg.SOLVER.WARMUP_ITERS),
            f"sample_{self.sample_num}",
        )
