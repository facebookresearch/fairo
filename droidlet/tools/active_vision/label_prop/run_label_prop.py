...
def propogate_label(
    ...
    for img_indx in range(image_range[0], image_range[1] + 1):
        ...
        try:
            ...
            print(
                f'{img_indx} out of bounds! Total images {len(os.listdir(os.path.join(root_path, "rgb")))}'
            )
            continue
        ...
        save_propagated_visual(gt_label, annot_img, os.path.join(out_dir, "lp_visuals"), out_indx)
            ...
            def copyover(in_dir, out_dir, p):
                for x in range(p + 1):
                    if os.path.isfile(os.path.join(in_dir, f"{x:05d}.npy")):
                        copyfile(
                            os.path.join(in_dir, f"{x:05d}.npy"),
                            os.path.join(out_dir, f"{x:05d}.npy"),
                        )
...
def sanity_check_traj(x):
    ...
    if int(gt) != len(reex_objects) or int(gt) != valid_objects:
        print(
            f"traj {tid} expected objects {gt}, actual {len(reex_objects)}, valid_objects {valid_objects}"
        )
        is_valid = False
                        ...
                        for eid in os.listdir(path.parent):
                            if eid.isdigit():
                                ...
                                def job_unit(path, eid, set_keys, od, jod, rp):
                                    ...
                                    propagate_dir(
                                        os.path.join(path.parent, eid), os.path.join(od, eid)
                                    )
