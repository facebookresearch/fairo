...
if [[ $USE_ROS -eq 1 ]]; then
    python3 ./remote_hello_robot_ros.py --ip $ip &> remote_hello_robot_ros.out &
else
    python3 ./remote_hello_robot.py --ip $ip &> remote_hello_robot.out &
fi
BGPID=$!
...
if [[ $USE_ROS -eq 1 ]]; then
    python3 ./remote_hello_realsense.py --ip $ip --ros &> remote_hello_realsense.out &
else
    python3 ./remote_hello_realsense.py --ip $ip --no-ros &> remote_hello_realsense.out &
fi
BGPID2=$!
...
python3 ./remote_hello_saver.py --ip $ip &> remote_hello_saver.out &
BGPID3=$!
