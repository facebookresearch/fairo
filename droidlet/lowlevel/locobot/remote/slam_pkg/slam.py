...
class Slam(object):
    ...
    def take_step(self, step_size):
        ...
        if not traversable[self.stg[1], self.stg[0]]:
            print("Obstacle in path. If planner is working correctly, this should never happen!!")
        ...
        if exec:
            ...
            print(f'translation failed because of an obstacle') 
            pp('robot_map_loc', robot_map_loc)
            ...
            # update map builder as obstacle 
            ...
            ob = [x for x in zip(*np.where(self.planner.fmm_dist == self.planner.fmm_dist.max()))]
