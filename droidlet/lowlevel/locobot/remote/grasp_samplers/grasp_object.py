...
import grasp_samplers.deeper_models as deeper_models
...
class GraspTorchObj(object):
    """This class contains functionality to wrap a torch grasp model with
    testing operations."""
    ...
    def __init__(self, model_path, transform=None):
        ...
        self.model = deeper_models.IfullRobHWNet()
        self.model.load_state_dict(torch.load(model_path))
        # self.recursion_change_bn(self.model)
        self.model.eval()
        self.image_size = 224
        if is_gpu:
    ...
    def convert_one_hot(self, labels):
        """Converts a list of grasp angle labels to a Torch tensor of one_hot
        labels.
    ...
    def get_default_transform(self):
        ...
        corresponds to the transform that was applied to the images during
        training.
