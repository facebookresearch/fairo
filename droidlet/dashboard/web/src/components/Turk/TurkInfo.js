...
class TurkInfo extends Component {
  ...
  calcCreativity(command) {
    ...
    this.state.commandCorpus.forEach((cmd) => {
      ...
      let lengthNorm =
        commandLength > comparisonCommandArray.length
          ? commandLength
          : comparisonCommandArray.length;
      let dist =
        minimumEditDistance.diff(commandArray, comparisonCommandArray)
          .distance /
        (lengthNorm * corpus_size * 0.1);
    ...
    creativityScore -=
      Math.pow(Math.abs(optimalCommandLength - commandLength), 2) * 1.5; // Take off points for long or short commands
    ...
    let avgCreativity =
      newCreativityScores.slice(1).reduce((a, b) => a + b) /
      newCreativityScores.slice(1).length;
    ...
    if (newCommandList.length > 0) {
      newCommandList.forEach((cmd) => {
        let lengthNorm =
          commandLength > cmd.length ? commandLength : cmd.length;
        let dist =
          minimumEditDistance.diff(commandArray, cmd).distance /
          (lengthNorm * newCommandList.length * 0.1);
    ...
    let avgDiversity =
      newDiversityScores.slice(1).reduce((a, b) => a + b) /
      newDiversityScores.slice(1).length;
    ...
    let stoplightScore =
      (Math.log10(avgCreativity) +
        Math.log10(avgDiversity) +
    ...
    let feedback = "";
    if (commandLength < optimalCommandLength - 1)
      feedback += "Try sending longer commands. ";
    else if (commandLength > optimalCommandLength + 1)
      feedback += "Try sending shorter commands. ";
    else if (creativityScore < 5)
      feedback += "Try sending more creative commands. ";
    if (quantityScore < 5) feedback += "Please send more commands. ";
    else if (diversityScore < 4)
      feedback += "Commands should be more different from one another. ";
