...
class StateManager {
  ...
  constructor() {
    ...
    this.processPointCloud = this.processPointCloud.bind(this);
  ...
  restart(url) {
    ...
    socket.on("pointcloud", this.processPointCloud);
  ...
  processPointCloud(msg) {
    this.refs.forEach((ref) => {
      if (ref instanceof World3D) {
        ref.setState({
          isLoaded: true,
          points: [...ref.state.points, ...msg.points],
          colors: [...ref.state.colors, ...msg.colors],
          base: msg.base,
        });
      }
    });
